...
import 'package:pina/screens/payment_method_screen.dart';
...
class _PricingMenuScreenState extends State<PricingMenuScreen> {
  ...
  // ── Fetch only individual.price and academy.price from API ──
      ...
      if (res.statusCode == 200) {
        ...
        for (final item in data) {
          ...
          if (planName.contains('individual')) {
            ...
            // Only price from API; validity & credits stay hardcoded
            final acad =
                _plans.firstWhere((p) => p.id == 'academy');
            acad.price = priceVal == 0 ? 'Contact Us' : '₹$priceVal';
          }
          // free_trial and organisation → never touched
        }
...
class _PlanCard extends StatelessWidget {
  ...
  const _PlanCard(
      {required this.plan, required this.isActive, required this.onTap});
